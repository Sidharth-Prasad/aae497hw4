...
casadi_int main_rocket_aero_forces(casadi_int argc, char* argv[]) {
  casadi_int *iw = 0;
  casadi_real w[71];
  ...
  casadi_int flag = rocket_aero_forces(arg, res, iw, w+36, 0);
  if (flag) return flag;
...
int main(int argc, char* argv[]) {
  if (argc<2) {
    /* name error */
  } else if (strcmp(argv[1], "rocket_aero_forces")==0) {
    return main_rocket_aero_forces(argc-2, argv+2);
  }
  fprintf(stderr, "First input should be a command string. Possible values: 'rocket_aero_forces'\n");
  return 1;
}
#ifdef __cplusplus
} /* extern "C" */
